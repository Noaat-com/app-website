...
export default function CouponsOffers({ params: { lang } }) {
    const [dict, setDict] = useState(null);

    useEffect(() => {
        const fetchDictionary = async () => {
            const d = await getDictionary(lang);
            setDict(d.CouponsOffersPage);
        };
        fetchDictionary();
    }, [lang]);
        ...
        <main className='container mx-auto px-5 py-10'>
            ...
            >
                <div className='p-8 border border-base-content/10 rounded-2xl'>
                    <h2 className='text-3xl font-bold mb-4'>{dict.h2_how_to_use}</h2>
                ...
                <div className='p-8 border border-base-content/10 rounded-2xl'>
                    <h2 className='text-3xl font-bold mb-4'>{dict.h2_types_of_offers}</h2>
                    <ul className='list-disc list-inside space-y-2'>
                        <li>{dict.offer1}</li>
                        <li>{dict.offer2}</li>
                        <li>{dict.offer3}</li>
